...
const Timer = React.memo(
  React.forwardRef<
    HTMLDivElement,
    { hours: number; minutes: number; seconds: number; className?: string }
  >(({ hours, minutes, seconds, className }, ref) => {
    ...
    return (
      <div
        ref={ref}
        className={cn(
          "flex items-center justify-center gap-0.5 rounded-md font-mono font-medium text-foreground bg-background",
          className
        )}
      >
        <span>{h1}</span>
        <span>{h2}</span>:<span>{m1}</span>
        <span>{m2}</span>:<span>{s1}</span>
        <span>{s2}</span>
      </div>
    );
  })
...
const VolumeControl = React.memo(
  React.forwardRef<
    HTMLDivElement,
    { volume: number; onVolumeChange: (value: number[]) => void }
      ...
      <div ref={ref} className="w-full flex flex-col gap-2">
        ...
        >
          <SelectTrigger id="mic-select" className="w-full">
            <SelectValue placeholder={t("Select a microphone")} />
          </SelectTrigger>
          <SelectContent>
            {mics.map((mic, index) => (
              <SelectItem key={mic.deviceId} value={mic.deviceId}>
                {mic.label || t("Microphone {{number}}", { number: index + 1 })}
              </SelectItem>
            ))}
          </SelectContent>
        </Select>
...
export const AudioRecorderWithVisualizer = ({
  ...
  const getMicrophoneStream = async () => {
    try {
      ...
      alert(
        t(
          "Microphone access was denied or the selected device is not available."
        )
      );
      return null;
    }
  ...
  const playAudio = (offset = webAudioRefs.current.pausedTime) => {
    ...
    sourceNode.onended = () => {
      if (webAudioRefs.current.sourceNode === sourceNode) pauseAudio(true);
    };
  ...
  const pauseAudio = (isEnded = false) => {
    ...
    webAudioRefs.current.pausedTime = isEnded
      ? 0
  ...
  const handlePlaybackToggle = () => {
    if (playbackStatus === "playing") pauseAudio();
    else playAudio();
  };
  ...
  const handleVolumeChange = (newVolume: number[]) => {
    ...
    if (webAudioRefs.current.gainNode && webAudioRefs.current.audioContext) {
      webAudioRefs.current.gainNode.gain.setValueAtTime(
        vol,
        webAudioRefs.current.audioContext.currentTime
      );
    }
  };
  const toggleLooping = () => {
  ...
  useEffect(() => {
    // ... (useEffect hooks remain the same, no text changes needed)
  ...
  useEffect(() => {
    const drawVisualizer = (dataArray: Uint8Array) => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        ...
        const smoothedHeight =
          prevBarHeightsRef.current[i] +
          (targetHeight - prevBarHeightsRef.current[i]) * smoothingFactor;
    ...
    const visualize = () => {
      ...
      const draw = () => {
        ...
        if (recordingStatus === "recording") {
          webAudioRefs.current.analyser.getByteTimeDomainData(
            timeDomainDataArray
          );
          let maxDeviation = 0;
        ...
        if (playbackStatus === "playing") {
          ...
          const newTime =
            webAudioRefs.current.pausedTime +
            (audioContext!.currentTime - startTime);
      ...
      <div className="flex w-full min-h-[40px] max-w-5xl items-center justify-center gap-2">
        {recordingStatus !== "inactive" ? (
          ...
          <>
            <Tooltip>
              <TooltipTrigger>
                <Button onClick={handlePlaybackToggle} size="icon">
              ...
              <TooltipContent>
                <p>{t(playbackStatus === "playing" ? "Pause" : "Play")}</p>
              </TooltipContent>
            </Tooltip>
            <Tooltip>
              <TooltipTrigger>
