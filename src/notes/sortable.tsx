...
interface KanbanContextProps<T> {
  ...
  columnIds: string[];
  activeId: any | null;
  setActiveId: (id: any | null) => void;
  findContainer: (id: any) => string | undefined;
  isColumn: (id: any) => boolean;
}
...
function Kanban<T>({
  ...
  const [activeId, setActiveId] = React.useState<any | null>(null);
  ...
  const isColumn = React.useCallback(
    (id: any) => columnIds.includes(id as string),
    [columnIds]
  );
...
export interface KanbanOverlayProps {
  ...
  children?:
    | React.ReactNode
    | ((params: { value: any; variant: "column" | "item" }) => React.ReactNode);
}
...
export interface SortableRootProps<T> {
  ...
  children: React.ReactNode;
  className?: string;
  onMove?: (event: { event: any; activeIndex: number; overIndex: number }) => void;
